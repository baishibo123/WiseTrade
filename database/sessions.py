...
def derive_sessions(conn: sqlite3.Connection, validate_against: Optional[str] = "XNYS") -> int:
    ...
    bounds = _package_rth_bounds(sorted(by_date), validate_against)
    ...
    if validate_against:
        for problem in _cross_check(rows, validate_against):
            logging.warning(f"calendar cross-check: {problem}")

    return len(rows)


def _package_rth_bounds(session_dates: list, calendar_name: Optional[str]) -> dict:
    """
    Exact (rth_open_exclusive, rth_close_inclusive) per date from the calendar
    package, for the dates the data says are sessions.

    Returns {} when the package is absent so the caller falls back to deriving
    the boundary from bar density. Only the *intraday* boundary is taken from
    the package; the session set itself stays data-derived (ADR-019).
    """
    if not calendar_name or not session_dates:
        return {}
    try:
...
def _cross_check(rows: list, calendar_name: str) -> list[str]:
    """
    Compare derived sessions against an exchange-calendar package.
    ...
    try:
        ...
        logging.info("exchange_calendars not installed; skipping calendar cross-check")
        return []
    ...
    for d in sorted(expected - derived):
        problems.append(f"{d} is a {calendar_name} session but has no bars -- missing data?")
    for d in sorted(derived - expected):
        problems.append(f"{d} has bars but is not a {calendar_name} session")
    return problems
...
class TradingCalendar:
    ...
    def __init__(self, sessions: list[Session]):
        self._sessions = sorted(sessions, key=lambda s: s.ext_open_ms)
        self._starts = [s.ext_open_ms for s in self._sessions]
    ...
    def session_for(self, ts_ms: int) -> Optional[Session]:
        ...
        if i < 0:
            return None
        s = self._sessions[i]
        return s if ts_ms <= s.ext_close_ms else None
    ...
    def periods_per_year(self, regular_hours_only: bool = True) -> float:
        """
        Bars per year, measured from the sessions actually present.

        Replaces analyzer.py's hardcoded 252 * 390. That constant happens to be
        close for regular hours -- measured 251.7 sessions/year x 390 -- but is
        wrong by ~2.4x once extended hours are included (947 bars/session),
        which mis-scales annualised volatility by sqrt(2.4) ~ 1.56 and with it
        every Sharpe ratio.
        """
        ...
        if regular_hours_only:
            bars = sum(s.n_bars_rth for s in self._sessions) / len(self._sessions)
        else:
